...
class SignalAnalyzer:
    ...
    def update(self, t_start, t_length, domain):
        ...
        if domain=='Time Trace':
            ...
            ymax, ymin = max(self.x_n), min(self.x_n)
            self.ax2.axis(xmin=self.t[n_start], xmax=self.t[n_stop], ymin=ymin - (0.1)*(ymax-ymin), ymax=ymax + (0.1)*(ymax-ymin))

        elif domain=='Frequency Spectrum':
            M = n_stop-n_start
            f, Sxx_sub = periodogram(self.x_n[n_start:n_stop], self.f_s, 'hamming', M)
